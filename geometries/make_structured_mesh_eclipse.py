...
nx = int(args['number_of_cells_x'])
ny = 1
nz = int(args['number_of_cells_y']) #this is actually the z dimension
grid_file_name = os.path.splitext(_get_variant_geo_file(variant))[0] + f"_GRID_{nx}_{ny}_{nz}".format() + ".GRDECL"
print(f"Writing GRDECL grid file '{grid_file_name}'")
with open(grid_file_name, "w") as grid_file:
    grid_file.write("DIMENS\n")
    grid_file.write(f"{nx} {ny} {nz} /\n\n".format())
    grid_file.write("COORD\n")
    for count, p in enumerate(filtered_lattice.points):
        if p[1]==0.0:
            grid_file.write(f"{p[0]} 0.0 0.0 {p[0]} 0.0 1000.0 \n".format())
    for count, p in enumerate(filtered_lattice.points):
        if p[1] == 0.0:
            grid_file.write(f"{p[0]} {thickness} 0.0 {p[0]} {thickness} 1000.0 \n".format())
    grid_file.write("/\n\n")
    grid_file.write("ZCORN\n")
    count = 0
    for k in reversed(range(nz)):
        for j in (range(ny)):
            for i in (range(nx)):
                #grid_file.write(f"{i} {j} {k} {(i) + (k) * (nx+1) + (j) * nx * ny} \n".format())
                grid_file.write(f"{-filtered_lattice.points[(i) + (k+1) * (nx+1) + (j) * nx * ny][1]} \n".format())
                #grid_file.write(f"{i+1} {j} {k} {(i+1) + (k) * (nx+1) + (j) * nx * ny} \n".format())
                grid_file.write(f"{-filtered_lattice.points[(i + 1) + (k+1) * (nx+1) + (j) * nx * ny][1]} \n".format())
            for i in range(nx):
                #grid_file.write(f"{i} {j} {k} {(i) + (k) * (nx+1) + (j) * nx * ny} \n".format())
                grid_file.write(f"{-filtered_lattice.points[(i) + (k+1) * (nx+1) + (j) * nx * ny][1]} \n".format())
                #grid_file.write(f"{i+1} {j} {k} {(i+1) + (k) * (nx+1) + (j) * nx * ny} \n".format())
        ...
        for j in range(ny):
            for i in range(nx):
                #grid_file.write(f"{i} {j} {k+1} {(i) + (k+1) * (nx+1) + (j) * nx * ny} \n".format())
                grid_file.write(f"{-filtered_lattice.points[(i) + (k) * (nx+1) + (j) * nx * ny][1]} \n".format())
                #grid_file.write(f"{i+1} {j} {k+1} {(i+1) + (k+1) * (nx+1) + (j) * nx * ny} \n".format())
                grid_file.write(f"{-filtered_lattice.points[(i + 1) + (k) * (nx+1) + (j) * nx * ny][1]} \n".format())
            for i in range(nx):
                #grid_file.write(f"{i} {j} {k+1} {(i) + (k+1) * (nx+1) + (j) * nx * ny} \n".format())
                grid_file.write(f"{-filtered_lattice.points[(i) + (k) * (nx+1) + (j) * nx * ny][1]} \n".format())
                #grid_file.write(f"{i+1} {j} {k+1} {(i+1) + (k+1) * (nx+1) + (j) * nx * ny} \n".format())
...
multnum_file_name = os.path.splitext(_get_variant_geo_file(variant))[0] + f"_MULTNUM_{nx}_{ny}_{nz}".format() + ".GRDECL"
print(f"Writing GRDECL MULTNUM file '{multnum_file_name}'")
dup_count = 0
write_count = 1
with open(multnum_file_name, "w") as multnum_file:
    multnum_file.write("MULTNUM\n")
    '''for cell_index in reversed(range(nx*ny*nz)):
        if cell_index > 0 :
            if filtered_physical_cell_indices[cell_index] == filtered_physical_cell_indices[cell_index-1]:
                dup_count += 1
            elif dup_count > 0:
                dup_count += 1
                multnum_file.write(f"{dup_count}*{filtered_physical_cell_indices[cell_index]} ".format())
                dup_count = 0
                write_count += 1
            else:
                multnum_file.write(f"{filtered_physical_cell_indices[cell_index]} ".format())
                write_count += 1
        else:
            if dup_count > 0:
                dup_count += 1
    ...
    for k in reversed(range(nz)):
        for j in (range(ny)):
            for i in (range(nx)):
                cell_index = ((i) + (k) * (nx) + (j) * nx * ny)
                next_cell_index = (((i+1) % nx) + (k) * (nx) + (j-(i+1)//nx) * nx * ny)
                if cell_index > 0:
                    if filtered_physical_cell_indices[cell_index] == filtered_physical_cell_indices[next_cell_index]:
                        dup_count += 1
                    elif dup_count > 0:
                        dup_count += 1
                        multnum_file.write(f"{dup_count}*{filtered_physical_cell_indices[cell_index]} ".format())
